...
watch(
    ...
    (newValue) => {
        formData.value.slug_name_product = StringService.toSlug(newValue);
    }
);
const errors = ref({});
    ...
    try {
        ...
        fileList.value  = images.flatMap((image: any) => 
            image.url_img.map((img: string) => ({
                name: img.replace("https://nongsannvp.store/vehiclebooking_be/storage/app/public/images/", ""), // Tên file từ mảng url_img
                status: 'done',
